...
function buildManifest(): SoldiersMediaAsset[] {
  ...
  for (const ex of EXERCISE_LIBRARY) {
    const published = (PILOT_EXERCISE_IDS as readonly string[]).includes(ex.id);
    rows.push(
  ...
  for (const id of BRAND_IDS) {
    rows.push(
      posterAsset("brand", id, true, true, {
        durationSec: 4,
        prompt: "Soldiers welcome hero: athlete looping a clean squat-to-stand in dark gold studio, 4s, no text.",
      }),
    );
  }

  const howtoPrompts: Record<(typeof HOWTO_IDS)[number], string> = {
    "log-meal": "Hands logging a meal on a dark phone in Soldiers app, 3s loop, no readable UI text.",
    "mix-whey": "Athlete mixing whey in a black shaker, gold rim light, 3s loop, no brand besides Soldiers.",
    "complete-set": "Athlete racking a barbell after a set, nod of completion, 3s loop, dark studio.",
  };
...
const publishedIndex = new Map(
  SOLDIERS_MEDIA_MANIFEST.filter((a) => a.status === "published").map((a) => [key(a.kind, a.entityId), a]),
);

export function manifestAsset(kind: SoldiersMediaKind, entityId: string): SoldiersMediaAsset | undefined {
  return publishedIndex.get(key(kind, entityId)) ?? SOLDIERS_MEDIA_MANIFEST.find((a) => a.kind === kind && a.entityId === entityId);
}
