/**
 * Ephemeral learning insights (kcal / protein / week hint).
 * typicalSleepHours is not today's sleep.
 */
...
function weightTrend7d(weights: AppState["weights"], date: string): number | null {
  if (weights.length < 2) return null;
  const cutoff = dateNDaysAgo(7, date);
...
function proteinAdherence7d(state: AppState, profile: Profile, date: string) {
  const base = nutritionGoals(profile);
  let sum = 0;
  for (let i = 0; i < 7; i += 1) {
    const day = dateNDaysAgo(i, date);
    const totals = dayNutritionTotals(state.meals ?? [], day);
    sum += Math.min(1, totals.proteinG / Math.max(1, base.proteinG));
  }
  return Math.round((sum / 7) * 100) / 100;
}

/** Recompute insights from persisted results — no persistence. */
export function computeLearningInsights(
  ...
  const proteinAdherence = proteinAdherence7d(state, profile, date);
  const trend = weightTrend7d(state.weights, date);
  ...
  if (routine.length > 0 && supplementAdherence30d < 0.5) {
    reasons.push(
      `Aderência de suplementos em ${monthAdh.pct}% no mês — marque as doses no horário.`,
    );
  }
  ...
  const todayCheck = state.dayCheckIns?.[date];
  const sleepH = todayCheck?.sleepHours;
  ...
  if (recentChecks.length >= 3) {
    const avgSleep = recentChecks.reduce((s, c) => s + c.sleepHours, 0) / recentChecks.length;
    const highStress = recentChecks.filter((c) => (c.stress ?? 0) >= 4).length;
    const highSoreness = recentChecks.filter((c) => (c.soreness ?? 0) >= 4).length;
    if (avgSleep < 6.5) {
      reasons.push(
        `Média de sono 7d: ${avgSleep.toFixed(1)}h — histórico de check-in aponta recuperação baixa.`,
      );
    }
    if (highStress >= 2) {
      reasons.push(
        `Estresse elevado em ${highStress}/7 check-ins — reduza intensidade mental do treino.`,
      );
    }
    if (highSoreness >= 2) {
      reasons.push(
        `Dor muscular alta em ${highSoreness}/7 check-ins — priorize mobilidade/deload.`,
      );
    }
  }
...
export function topLearningInsight(state: AppState, date = todayKey()): string | null {
