/**
 * Materialize LivingPlanSnapshot from already-computed safety + decisions.
 * Does not call Context / Safety / Decision engines — assembleDecisionContext owns that.
 */
...
import { trafficScoreForReadiness, type RecoverySnapshot } from "@/lib/engine/recovery";
import { computeLearningInsights, type LearningSnapshot } from "@/lib/engine/learning";
...
export function materializeLivingPlan(opts: {
  state: AppState;
  date: string;
  ...
  const { state, date, snapshot, safety, bundle, day, recovery, learning } = opts;
  ...
  const skipsCafe =
    profile.skipBreakfast === true || (Array.isArray(activeSlots) && !activeSlots.includes("cafe"));
  if (skipsCafe) {
    const n =
  ...
  const recoveryScore = recovery
    ? trafficScoreForReadiness(recovery.readiness, recovery.score)
  ...
  const behaviorLoop = learning?.behavior ?? runBehaviorLoop(state, { date });
  const rankInput: import("@/lib/content/recommend").ContentRankInput = {
    context: { goal: profile.goal, recovery: { level: snapshot.recovery.level } },
    decisions: { trainingMode: bundle.trainingMode, primaryAction: bundle.primaryAction },
    behavior: {
      triggers: behaviorLoop.triggers.map((t) => ({ key: t.key, active: t.active })),
    },
  };
  ...
  const primaryConf =
    primaryDecision?.confidence ?? modeDecision?.confidence ?? snapshot.confidenceBase;
  ...
  if (prev) {
    ...
    if (prev.blocker?.key !== livingSnapshot.blocker?.key) {
      diffs.push(
        `Bloqueio: ${prev.blocker?.label ?? "—"} → ${livingSnapshot.blocker?.label ?? "—"}`,
      );
    }
