...
import {
  runBehaviorLoop as runBehaviorLoopImpl,
  applyBehaviorOutcome as applyBehaviorOutcomeImpl,
...
export { extractUserPatterns, patternInsights };
...
/** Phase 4 Behavior Engine facade. */
export function runBehaviorLoop(
  state: AppState,
  opts?: Parameters<typeof runBehaviorLoopImpl>[1],
): BehaviorLoopResult {
  return runBehaviorLoopImpl(state, opts);
}
...
export function applyBehaviorOutcome(
  ...
  return applyBehaviorOutcomeImpl(profile, type, success);
}

export type { BehaviorLoopResult };
